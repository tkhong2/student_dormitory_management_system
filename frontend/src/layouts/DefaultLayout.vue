<template>
  <v-app style="background-color: #f8fafc">
      ...
      <div class="d-flex flex-column fill-height">
        ...
        <router-link to="/admin" class="d-flex align-center ga-2 pa-8 text-decoration-none border-b">
          ...
          <div>
            ...
            <div class="text-caption font-weight-bold opacity-60 uppercase tracking-widest">Quản trị viên</div>
          </div>
        </router-link>
        ...
        <div class="flex-grow-1 overflow-y-auto pa-4 scrollbar-hide">
          <div v-for="section in menu" :key="section.label" class="mb-6">
            <div class="px-4 text-caption font-weight-bold text-medium-emphasis uppercase tracking-widest mb-2">
              {{ section.label }}
            </div>
        ...
        <div class="user-info-fixed">
          <v-card flat color="grey-darken-4" class="pa-4 rounded-xl d-flex align-center ga-3">
            ...
            <div class="min-w-0 flex-grow-1">
              <div class="text-body-2 font-weight-black text-white text-truncate">Administrator</div>
    ...
    <!-- Topbar -->
    <v-app-bar flat color="white" height="64" elevation="0" style="border-bottom: 1px solid #f1f5f9;">
      <div class="d-flex align-center w-100 px-4">
        <v-btn icon="mdi-menu" variant="text" size="small" @click="drawer = !drawer" class="mr-3" />
        
        <div class="d-none d-sm-flex align-center" style="gap: 6px;">
          <span class="text-caption text-medium-emphasis">Trang chủ</span>
          <v-icon size="12" class="text-medium-emphasis">mdi-chevron-right</v-icon>
          <span class="text-caption font-weight-bold">{{ $route.meta?.title || $route.name }}</span>
        </div>
...
const menu = [
  ...
  { label: 'Cơ sở vật chất', items: [
    { title: 'Tòa nhà', icon: 'mdi-office-building-outline', to: '/admin/buildings' },
    { title: 'Phòng ở', icon: 'mdi-door-open', to: '/admin/rooms' },
  ]},
  { label: 'Sinh viên', items: [
    { title: 'Quản lý Sinh viên', icon: 'mdi-school-outline', to: '/admin/students' },
    { title: 'Hợp đồng', icon: 'mdi-file-sign', to: '/admin/contracts' },
  ]},
  { label: 'Tài chính', items: [
    { title: 'Hóa đơn', icon: 'mdi-receipt-text-outline', to: '/admin/billing' },
    { title: 'Bảo trì', icon: 'mdi-wrench-outline', to: '/admin/maintenance' },
  ]},
]
