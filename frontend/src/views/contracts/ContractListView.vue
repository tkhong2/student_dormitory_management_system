<template>
  <div>
    <div class="d-flex justify-space-between align-center flex-wrap ga-3 mb-4">
      ...
      <v-btn color="primary" prepend-icon="mdi-plus" variant="flat" @click="openCreate">Tạo hợp đồng</v-btn>
    </div>
      ...
      <v-col cols="6" md="3">
        <v-card class="pa-4" style="border:1px solid #e5e7eb">
          <div class="d-flex align-center ga-3">
            ...
            <div><div class="text-h6 font-weight-bold">{{ countStatus('Hiệu lực') }}</div><div class="text-caption text-medium-emphasis">Đang hiệu lực</div></div>
          </div>
        </v-card>
      </v-col>
      <v-col cols="6" md="3">
        <v-card class="pa-4" style="border:1px solid #e5e7eb">
          <div class="d-flex align-center ga-3">
            ...
            <div><div class="text-h6 font-weight-bold">{{ countStatus('Sắp hết hạn') }}</div><div class="text-caption text-medium-emphasis">Sắp hết hạn</div></div>
          </div>
        </v-card>
      </v-col>
      <v-col cols="6" md="3">
        <v-card class="pa-4" style="border:1px solid #e5e7eb">
          <div class="d-flex align-center ga-3">
            ...
            <div><div class="text-h6 font-weight-bold">{{ countStatus('Đã chấm dứt') }}</div><div class="text-caption text-medium-emphasis">Đã chấm dứt</div></div>
          </div>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-alert v-else-if="error" type="error" variant="tonal" class="mb-4">
      {{ error }}
      <template #append><v-btn variant="text" size="small" @click="loadContracts">Thử lại</v-btn></template>
    </v-alert>
    <v-card v-else style="border:1px solid #e5e7eb">
      <v-data-table :headers="headers" :items="contracts" items-per-page="10">
        <template #item.studentName="{ item }">
          ...
          <div class="text-caption text-medium-emphasis">{{ item.studentCode }}</div>
        </template>
        <template #item.price="{ item }"><span class="font-weight-bold">{{ formatPrice(item.price) }}</span></template>
        ...
        <template #item.status="{ item }">
          <v-chip :color="statusColor(item.displayStatus)" size="x-small" variant="tonal">{{ item.displayStatus }}</v-chip>
        </template>
        <template #item.actions="{ item }">
          <v-btn icon="mdi-pencil-outline" size="small" variant="text" color="primary" title="Chỉnh sửa" @click="openEdit(item)" />
          <v-btn icon="mdi-close-circle-outline" size="small" variant="text" color="error" title="Chấm dứt" :disabled="item.status === 'Terminated'" @click="terminateContract(item)" />
          <v-btn icon="mdi-delete-outline" size="small" variant="text" color="error" title="Xóa" @click="confirmDelete(item)" />
        </template>
      </v-data-table>
    </v-card>
    ...
    <v-dialog v-model="dialog" max-width="680" persistent>
      <v-card class="pa-6">
        <h2 class="text-h5 font-weight-bold mb-6">{{ editTarget ? 'Chỉnh sửa hợp đồng' : 'Tạo hợp đồng mới' }}</h2>
        <v-row>
          <v-col cols="12" sm="4"><v-text-field v-model="form.code" label="Mã hợp đồng *" :error-messages="formErrors.code" /></v-col>
          <v-col cols="12" sm="4"><v-text-field v-model="form.studentCode" label="Mã sinh viên *" :error-messages="formErrors.studentCode" /></v-col>
          <v-col cols="12" sm="4"><v-text-field v-model="form.studentName" label="Tên sinh viên *" :error-messages="formErrors.studentName" /></v-col>
          <v-col cols="12" sm="6"><v-text-field v-model="form.roomNumber" label="Phòng *" :error-messages="formErrors.roomNumber" /></v-col>
          <v-col cols="12" sm="6"><v-text-field v-model.number="form.price" label="Giá thuê *" suffix="đ/tháng" type="number" :error-messages="formErrors.price" /></v-col>
          <v-col cols="12" sm="4"><v-text-field v-model="form.startDate" label="Ngày bắt đầu *" type="date" :error-messages="formErrors.startDate" /></v-col>
          <v-col cols="12" sm="4"><v-text-field v-model="form.endDate" label="Ngày kết thúc *" type="date" :error-messages="formErrors.endDate" /></v-col>
          <v-col cols="12" sm="4"><v-select v-model="form.status" label="Trạng thái" :items="statusOptions" item-title="label" item-value="value" /></v-col>
        </v-row>
        <div class="d-flex justify-end ga-3 mt-4">
          <v-btn variant="text" :disabled="saving" @click="closeDialog">Hủy</v-btn>
          <v-btn color="primary" :loading="saving" @click="saveContract">{{ editTarget ? 'Lưu thay đổi' : 'Tạo hợp đồng' }}</v-btn>
        </div>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="420">
      <v-card class="pa-6">
        ...
        <div class="d-flex justify-end ga-3 mt-4">
          ...
          <v-btn color="error" :loading="saving" @click="deleteContract">Xóa</v-btn>
        </div>
      </v-card>
    </v-dialog>
...
function defaultForm() {
  return { code: '', studentId: '', studentCode: '', studentName: '', roomId: '', roomNumber: '', price: 0, startDate: '', endDate: '', status: 'Active' }
}
...
async function loadContracts() {
  ...
  error.value = null
  try {
    contracts.value = (await contractService.getAll()).map((item) => ({ ...item, displayStatus: displayStatus(item) }))
  } catch (err) {
...
function statusColor(status) {
  return { 'Hiệu lực': 'success', 'Sắp hết hạn': 'warning', 'Đã gia hạn': 'info', 'Hết hạn': 'grey', 'Đã chấm dứt': 'error' }[status] || 'grey'
}
...
function openEdit(item) {
  ...
  form.value = { ...item, startDate: item.startDate.slice(0, 10), endDate: item.endDate.slice(0, 10) }
  formErrors.value = {}
  dialog.value = true
}
...
function validate() {
  ...
  if (!form.value.endDate || form.value.endDate < form.value.startDate) errors.endDate = 'Ngày kết thúc không hợp lệ'
...
async function terminateContract(item) {
  saving.value = true
  try {
    await contractService.update(item.id, payload({ ...item, status: 'Terminated' }))
